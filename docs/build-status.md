...
| OKX instrument metadata | Partial | A versioned SQLite cache plus typed list/refresh APIs persist tradable SWAP contract value/currency, lot/min/tick sizes, precision, and state. Strategy/position selectors and operator-unit conversion are the next UI integration step. |
