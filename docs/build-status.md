...
| Notification delivery | Built | The legacy support/resistance and standalone fluctuation alerts are removed. A bounded daemon routes only operator lifecycle audits to configured LINE and Gmail transports. SQLite stores the consumer cursor, per-channel acknowledgements, health, failure evidence, and bounded exponential retry state. The Traditional Chinese management page shows channel readiness/backlog and provides an explicitly confirmed real test without exposing credentials or destinations. |
