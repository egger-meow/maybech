...
| Canonical position-rule model | Partial | Strategy defaults and logical-position conditions persist schema-v1 purpose/style/trigger/action/parameters/evidence definitions through shared normalization and migrations. Absolute, fixed-price, fixed-percent, and evidence targets materialize into side-correct prices. Confirmed entry fills rematerialize relative rules; owned stop changes use verified exchange amendment. Staged take-profit reductions are bounded to initial quantity, complete once on confirmed allocation, and expose a running remainder. Authoring UI, automatic break-even, and trailing state remain. |
