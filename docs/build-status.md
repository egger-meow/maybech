...
| Manual logical-position open | Partial | Position Management provides a cached searchable selector, local replay price prefill, display/API quantities, notional and stop-PnL preview. Confirmed Simulation opens persist `source=manual`, initial stop/take-profit, allocation, and audit atomically. Demo submits a real order through the shared entry-submission path (risk approval, execution-ingestion-readiness gate, protection verification, emergency-close-on-failure). Live Safe and Live Armed remain intentionally blocked from manual open. |
