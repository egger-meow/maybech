...
| Logical position units | Partial | SQLite persistence, per-unit close conditions, account-wide net-position reconciliation, exact-gap external import, immutable entry/close/protective-algo ownership, confirmed stop amendments, out-of-order fill allocation, and manual close API are implemented. Generic rule edits cannot desynchronize an owned stop. Break-even operations remain incomplete. |
