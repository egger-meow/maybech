...
| Fixed-risk position design | Partial | The non-mutating risk-quote contract and Market Analysis calculator support fixed-loss stop derivation and chart-anchored position sizing. Results are tick/lot aligned, cannot exceed allowed loss, disclose unused risk, and include a persisted-rule-shaped stop proposal with evidence. Direct reviewed promotion into strategy defaults or per-position overrides is not built. |
...
| SQLite schema management | Partial | `TradeStore` and `ExecutionCursorStore` record version `1`, `AccountRiskStore` records version `3`, `AuditEventStore` records version `4`, `StrategyStore` records version `3`, and `LogicalPositionStore` records version `6` through explicit migration paths. All default to `MAYBECH_DB_PATH`. |
