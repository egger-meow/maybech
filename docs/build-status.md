...
| Notification delivery | Partial | The legacy support/resistance and standalone fluctuation alerts are removed. A bounded daemon routes only operator lifecycle audits to configured LINE and Gmail transports. SQLite stores the consumer cursor and per-channel acknowledgements, so restarts resume missed work and partial delivery does not duplicate a successful sibling channel. Persisted failure health and browser controls remain incomplete. |
...
| SQLite schema management | Partial | `TradeStore` and `ExecutionCursorStore` record version `1`, `AuditEventStore` records version `3`, `StrategyStore` records version `3`, and `LogicalPositionStore` records version `6` through explicit migration paths. All default to `MAYBECH_DB_PATH`. |
