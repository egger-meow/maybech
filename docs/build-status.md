...
| Market Intelligence Layer | Partial | Phase 0, Phase 1, and Phase 2 are complete: 15 metrics (OKX price/funding/open-interest, DefiLlama stablecoin market cap, CoinGecko globals, Fear & Greed, MVRV ratio + Z-Score) across sentiment/valuation/price_breadth/derivatives/liquidity pillars, persisted with freshness/provider-health tracking, exposed through typed `/market/metrics`, `/market/series/{id}`, `/market/providers/status`, and rendered on a redesigned `frontend/app/analysis/overview/page.tsx` (pillar cards, per-metric freshness badges, provider-health strip, timeframe-controlled history charts with hover tooltips, honest empty states for `holder_behavior` and not-yet-accumulated history). Live-verified end to end against real providers in the browser, including restart persistence. See `docs/market-intelligence.md`. Derived evidence (Phase 3), regime assessment (Phase 4), and the holder-behavior pillar (blocked on Coin Metrics catalog confirmation) remain planned. |
