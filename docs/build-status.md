...
| Market Intelligence Layer | Partial | Phase 0 (feasibility audit, metric catalog, provider capability matrix) and Phase 1 (domain models, metric registry, provider base contract with retry/backoff, SQLite-persisted observations and provider sync-run history, freshness classification, typed `/market/metrics`, `/market/metrics/{id}`, `/market/series/{id}`, `/market/providers/status` endpoints) are complete for three providers (alternative.me Fear & Greed, bitcoin-data.com MVRV Z-Score, CoinGecko global market/dominance). `MarketIntelligenceSyncService` runs in every mode without exchange dependency; a provider failure degrades only its own metrics. See `docs/market-intelligence.md`. Derived evidence, regime assessment, holder-behavior/liquidity pillars, and the Market Overview UI redesign remain planned (Phases 2-4). |
