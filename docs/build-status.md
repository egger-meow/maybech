...
| Fixed-risk position design | Partial | The non-mutating risk-quote contract and Market Analysis calculator support fixed-loss stop derivation and chart-anchored position sizing. Results are tick/lot aligned, cannot exceed allowed loss, and disclose unused risk. Confirmed revision-bound APIs can promote a server-recalculated stop into one single-instrument strategy default or an exact-size logical-position override; owned live stops still require confirmed exchange amendment. The analysis page now exposes explicit position promotion and reports API rejection when reviewed sizing would silently resize existing exposure. Strategy-default promotion UI remains. |
| Canonical position-rule model | Partial | Strategy defaults and logical-position conditions persist schema-v1 purpose/style/trigger/action/parameters/evidence definitions through shared normalization and migrations. Absolute, fixed-price, fixed-percent, and evidence targets materialize into side-correct prices. Confirmed entry fills rematerialize relative rules; owned stop changes use verified exchange amendment. Staged take-profit reductions are bounded to initial quantity, complete once on confirmed allocation, and expose a running remainder. Position Management now configures automatic fee/slippage-adjusted break-even parameters and visibly reports configured/armed/applied state; trailing state remains. |
