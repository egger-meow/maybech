...
## Runtime Mode Milestone

First-class `simulation`, `demo`, `live_safe`, and `live_armed` modes are built
across CLI startup, credential routing, preflight, API/OpenAPI, and the dashboard
banner. Demo uses the real executor, fills, protection, recovery, and
reconciliation path; the bounded OKX Demo lifecycle verifier remains the release
proof before real-money operation.
...
| Notification delivery | Built | The legacy support/resistance and standalone fluctuation alerts are removed. A bounded daemon routes only operator lifecycle audits to configured LINE and Gmail transports. SQLite stores the consumer cursor, only incomplete-event channel acknowledgements, health, failure evidence, and bounded exponential retry state; advancing the cursor compacts completed acknowledgements transactionally. The Traditional Chinese management page shows channel readiness/backlog and provides an explicitly confirmed real test without exposing credentials or destinations. |
| Account snapshot | Partial | Current API exposes account summary, positions, and orders snapshots. Each daemon-produced snapshot includes an explicit UTC observation timestamp so freshness-sensitive trailing logic can fail closed. |
| BTC regime state | Partial | `BTCRegimeService` publishes regime state and `/market/btc-regime` exposes the latest snapshot. |
| Market Intelligence Layer | Partial | Phase 0 through Phase 4 are complete, plus two post-Phase-4 addenda: 25 metrics (OKX price/funding/open-interest, DefiLlama stablecoin market cap, CoinGecko globals, market breadth (top-100 advancing %), Fear & Greed plus 7d/30d average/percentile/days-since-extreme, MVRV ratio + Z-Score + percentile, stablecoin mcap 7d/30d change, and a BTC price/open-interest quadrant regime) across sentiment/valuation/price_breadth/derivatives/liquidity pillars, persisted with freshness/provider-health tracking, exposed through typed `/market/metrics`, `/market/series/{id}`, `/market/providers/status`, and rendered on a redesigned `frontend/app/analysis/overview/page.tsx`. `GET /market/regime?at=` returns a six-pillar regime map (state/confidence/summary/evidence per pillar, deliberately no composite score) computed on demand from persisted observations bounded by an optional historical timestamp, so any assessment at time T is exactly reproducible with no new mutable table. Every pillar except `holder_behavior` (blocked on Coin Metrics catalog confirmation) now has an assessed state; `liquidity` still reports `unavailable` until 7+ days of history accumulate. Live-verified end to end against real providers in the browser, including restart persistence and honest degradation of window-dependent derived metrics and regime pillars on a fresh database. See `docs/market-intelligence.md`. Wiring research modules to consume historical regime context, the holder-behavior pillar, and Phase 5 (optional paid-provider expansion) remain planned. |
| Open interest history | Built | `GET /market/open-interest-history` (`frontend/app/analysis/open-interest`) lets the operator pick any OKX swap instrument via the shared `InstrumentSelector` and view its real historical open interest (contracts, settlement currency, USD) at 5m/1H/1D granularity, sourced live from OKX with no Maybech-side persistence. Always queries OKX's production environment regardless of `OKX_FLAG` — building this surfaced that OKX's Demo Trading environment reports a separate, unconstrained aggregate open interest (~500x production at spot-check) unlike mark price/funding, which it mirrors realistically; the fix is scoped to this endpoint only, and a follow-up task (not yet started) covers the same issue for the existing demo-mode `okx_btc_oi_usd`/`okx_oi_weighted_funding`/derived-metric/regime-pillar chain. Unavailable in simulation mode. |
| Market analysis research | Built | A bounded, cached `GET /market/analysis/support-resistance` endpoint exposes extrema clusters with timeframe, touch, volume, wick, recency, ATR, invalidation-distance, BTC-regime alignment, freshness, and candle-quality evidence. Candle caches enforce row/market limits; analysis windows are reused and incrementally rescanned. The UI explicitly presents fresh/partial/stale/unavailable, proposed, invalidated, and manual-review states. Stale, missing, invalidated, or BTC-conflicting research is rejected by both strategy and position promotion APIs; results remain non-executable without a reviewed transition. |
| Fixed-risk position design | Built | The non-mutating risk-quote contract and Market Analysis calculator support fixed-loss stop derivation and chart-anchored position sizing. The loss budget reserves entry/exit fees and two-sided slippage before deriving the stop or lot-aligned size; responses disclose price loss, modeled cost, total loss, and unused risk, and reject any result exceeding the allowance. Reviewed quotes promote through revision-bound strategy or exact-size position paths; owned live stops require confirmed exchange amendment. |
| Canonical position-rule model | Partial | Strategy defaults and logical-position conditions persist schema-v1 purpose/style/trigger/action/parameters/evidence definitions through shared normalization and migrations. Absolute, fixed-price, fixed-percent, evidence, break-even, and trailing activation rules materialize into side-correct prices. Staged take-profit reductions and trailing take-profit retracements allocate confirmed reductions once. Optional trailing stops persist their favorable water mark across restart, tighten monotonically through the confirmed protection-amend path, and fail closed on missing/stale observations. Strategy and Position Management expose typed trailing semantics, activation, distance, timeframe, freshness, reduction controls, lifecycle state, water/candidate evidence, and chart overlays. |
...
| Strategy Management page | Built | The responsive frontend creates, edits, inspects, enables, disables, and safely deletes persisted strategies. Default stop-loss supports entry-relative percent or fixed price; take-profit supports percent/fixed targets, full close, or staged initial/remaining reductions; automatic break-even exposes activation, entry/exit fees, slippage, and lock-in. Typed bounds and aggregate staged fractions are checked before revision-protected persistence. Primary/child signal expressions retain nested AND/OR editing, explicit saved state, readiness, and decision evidence. Enable and deletion remain dedicated confirmed transitions. |
| Position Management page | Built | The responsive frontend separates logical units from OKX net snapshots, renders typed K-line overlays, and authors per-position percent/fixed stop-loss, full/staged take-profit, automatic break-even, and trailing rules. Owned-stop price and typed metadata changes share the confirmed exchange-amend lifecycle. The page also shows protection/allocation/lifecycle evidence and confirms close/reduce commands. |
| Manual logical-position open | Built | Position Management provides a cached searchable selector, local replay price prefill, display/API quantities, notional and stop-PnL preview. Confirmed Simulation opens persist `source=manual`, initial stop/take-profit, allocation, and audit atomically. Demo and an armed, entries-enabled Live Armed runtime both submit a real order through the shared entry-submission path (risk approval, execution-ingestion-readiness gate, protection verification, emergency-close-on-failure). Live Safe remains intentionally blocked from manual open; an unarmed or entries-disabled Live Armed runtime is blocked the same way until armed. |
| Existing-position recovery | Built | Every account snapshot compares OKX net positions with exchange-backed logical units. Clear startup/external increases create separate `source=recovery` units with audit evidence and manual-review/protection-required state. Position Management provides a guarded adoption flow that requires a side-correct stop, balanced OKX quantity, and exchange proof of an independently owned reduce-only stop before clearing review. External reductions never guess a unit or use that shortcut; they mark all affected units for manual review once per changed reconciliation signature. Pending opens and Simulation units cannot be double-counted as recovery exposure. |
...
| SQLite schema management | Partial | `TradeStore` and `ExecutionCursorStore` record version `1`, `AccountRiskStore` records version `3`, `AuditEventStore` records version `4`, `StrategyStore` records version `5`, and `LogicalPositionStore` records version `7` through explicit migration paths. All default to `settings.MAYBECH_DB_PATH`, which is pinned per runtime mode at startup (`DEMO_MAYBECH_DB_PATH` for Demo, `MAYBECH_DB_PATH` otherwise). |
...
| Runtime preflight | Built | Importing configuration never arms orders. Explicit Demo/Live modes disarm first, validate their applicable credentials, account level, `net_mode`, reconciliation, risk, strategies, protection, and SWAP precision, then arm only order-capable modes or abort startup. `/runtime/preflight` exposes the mode and applicable checks. |
...
| Account risk envelope | Built | One versioned SQLite record owns maximum order notional, gross account exposure, leverage, and a non-empty allowed-instrument boundary. Live startup and every entry reject targets outside that allowlist. The Traditional Chinese Risk Limits page edits the real contract with a cached searchable SWAP selector, unsaved-state, validation, entry-gate blocking, stale-write protection, explicit confirmation, and atomic mutation audit evidence. |
| Live-readiness UI diagnostics | Built | The Traditional Chinese runtime banner distinguishes missing, ready, and not-yet-verified checks. A missing risk envelope names all numeric fields, the account allowlist, and enabled state; allowlist/strategy mismatches, instrument cache, per-strategy API size/slippage/protective stop, OKX account level, and `net_mode` are shown separately. Strategy Management filters its searchable target selector to the account boundary and explains missing or mismatched risk state inline. |
| Entry kill switch | Built | Entries default to disabled in SQLite and have a separate process-local arm from reduce-only closes; enable is rejected outside an armed live process. Once enabled, the persisted state survives restarts and is restored automatically once the process re-arms. Confirmed enable/kill APIs serialize against strategy submission; kill persists first, resolves accepted orders by exchange or client ID, requests cancellation only for `pending_open` units, and reports partial failures without re-enabling. |
