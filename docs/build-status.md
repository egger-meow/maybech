...
| Logical position units | Partial | SQLite persistence, per-unit close conditions, account-wide net-position reconciliation, exact-gap external import, immutable entry/close/protective-algo ownership, out-of-order fill allocation, and manual close API are implemented. Break-even operations remain incomplete. |
