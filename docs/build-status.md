...
| Market analysis research | Partial | A bounded, cached `GET /market/analysis/support-resistance` endpoint exposes extrema clusters with timeframe, touch, volume, wick, recency, ATR, invalidation-distance, BTC-regime alignment, freshness, and candle-quality evidence. The responsive Market Analysis page renders K-lines, level markers, evidence, and explicit fresh/partial/unavailable state. Operators can select one level as a non-executable stop anchor and explicitly promote an exact-size reviewed quote to an open logical position. Incremental recalculation beyond the short response cache remains incomplete. |
