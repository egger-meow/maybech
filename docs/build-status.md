...
| SQLite schema management | Partial | `TradeStore` and `ExecutionCursorStore` record version `1`, `AccountRiskStore` records version `3`, `AuditEventStore` records version `4`, `StrategyStore` records version `5`, and `LogicalPositionStore` records version `7` through explicit migration paths. All default to `MAYBECH_DB_PATH`. |
