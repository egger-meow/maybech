...
| Logical position units | Partial | SQLite persistence, per-unit close conditions, account-wide net-position reconciliation, exact-gap external import, immutable entry/close order ownership, out-of-order fill allocation, and manual close API are implemented. Break-even operations remain incomplete. |
...
| SQLite schema management | Partial | `TradeStore` and `ExecutionCursorStore` record version `1`, `AuditEventStore` records version `2`, `StrategyStore` records version `3`, and `LogicalPositionStore` records version `5` through explicit migration paths. All default to `MAYBECH_DB_PATH`. |
| Live order protection | Partial | Strategy entries are slippage-capped FOK orders using SDK-supported `attachAlgoOrds`; success requires a complete fill plus exact active child-algo proof. Any anomalous partial fill cancels the remainder and kills entries. A fully filled entry without proven child protection disables entries and submits a persisted, restart-retryable emergency close. Imported/recovered units create independently sized stops that are rechecked before every entry. Protective-stop trigger/cancellation allocation remains incomplete. |
...
1. Own protective-stop trigger, resize, and cancellation lifecycle.
2. Complete demo-account execution and restart verification before real money.
