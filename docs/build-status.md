...
| OKX instrument metadata | Partial | A versioned SQLite cache plus typed list/refresh APIs persist tradable SWAP contract value/currency, lot/min/tick sizes, precision, and state. Strategy instruments and all signal targets use a generated-contract searchable selector; manual-open integration and operator-unit conversion remain. |
