...
| Market analysis research | Built | A bounded, cached `GET /market/analysis/support-resistance` endpoint exposes extrema clusters with timeframe, touch, volume, wick, recency, ATR, invalidation-distance, BTC-regime alignment, freshness, and candle-quality evidence. Candle caches enforce row/market limits; analysis windows are reused for context-only changes and refreshed through a small overlap request. Pivot discovery rescans only the changed boundary, reuses prior candidates, and deterministically matches full discovery for unchanged data. The response reports API, state-capacity, and pivot-scan evidence; results remain explicitly non-executable research. |
| Fixed-risk position design | Partial | The non-mutating risk-quote contract and Market Analysis calculator support fixed-loss stop derivation and chart-anchored position sizing. Results are tick/lot aligned, cannot exceed allowed loss, and disclose unused risk. Market Analysis can explicitly promote a server-recalculated quote into a matching single-instrument strategy default or an exact-size logical-position override. Both paths bind the reviewed revision; enabled strategies are disabled for review, unsafe position resizing is rejected, and owned live stops still require confirmed exchange amendment. |
