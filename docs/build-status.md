...
| OKX instrument metadata | Partial | A versioned SQLite cache plus typed list/refresh/size-quote APIs persist tradable SWAP sizing data. Strategy instruments and signal targets use searchable selectors; strategy sizing is entered as operator-facing base quantity and displays derived OKX contracts/notional. Manual-open and position PnL integration remain. |
