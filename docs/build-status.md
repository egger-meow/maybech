...
| SQLite schema management | Partial | `TradeStore` and `ExecutionCursorStore` record version `1`, `AuditEventStore` records version `4`, `StrategyStore` records version `3`, and `LogicalPositionStore` records version `6` through explicit migration paths. All default to `MAYBECH_DB_PATH`. |
