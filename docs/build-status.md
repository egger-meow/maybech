...
| Market analysis research | Partial | A bounded, cached `GET /market/analysis/support-resistance` endpoint exposes extrema clusters with timeframe, touch, volume, wick, recency, ATR, invalidation-distance, BTC-regime alignment, freshness, and candle-quality evidence. Candle caches now enforce row/market limits; analysis windows are bounded, reused for context-only changes, and refreshed through a small overlap request instead of refetching full history. The response reports fetch/calculation mode and capacity. Pivot discovery still recalculates the bounded window and remains the next incremental step. |
