...
| Market Intelligence Layer | Partial | Phase 0 (feasibility audit, metric catalog, provider capability matrix) and Phase 1 (domain models, metric registry, provider base contract with retry/backoff, SQLite-persisted observations and provider sync-run history, freshness classification, typed `/market/metrics`, `/market/metrics/{id}`, `/market/series/{id}`, `/market/providers/status` endpoints) are complete. Phase 2 backend is complete: OKX price/funding/open-interest and DefiLlama total stablecoin market cap providers added (14 registered metrics total across sentiment/valuation/price_breadth/derivatives/liquidity pillars); OKX-scoped metrics report `enabled=false` and are skipped (not failed) in simulation mode. Live-verified against real OKX/CoinGecko/DefiLlama/alternative.me/bitcoin-data.com data, including a restart-persistence check. See `docs/market-intelligence.md`. Phase 2's Market Overview UI redesign, derived evidence (Phase 3), regime assessment (Phase 4), and the holder-behavior pillar (blocked on Coin Metrics catalog confirmation) remain planned. |
