...
| Market Intelligence Layer | Partial | Phase 0 through Phase 4 are complete, plus a post-Phase-4 addendum: 24 metrics (OKX price/funding/open-interest, DefiLlama stablecoin market cap, CoinGecko globals, Fear & Greed, MVRV ratio + Z-Score, five Phase 3 derived-evidence metrics — annualized OKX funding, BTC MVRV percentile, stablecoin mcap 7d/30d change, BTC price/open-interest quadrant regime — plus four Fear & Greed rolling-context metrics: 7d/30d average, percentile, and days since the last extreme reading) across sentiment/valuation/price_breadth/derivatives/liquidity pillars, persisted with freshness/provider-health tracking, exposed through typed `/market/metrics`, `/market/series/{id}`, `/market/providers/status`, and rendered on a redesigned `frontend/app/analysis/overview/page.tsx`. Phase 4 added `GET /market/regime?at=`: a six-pillar regime map (state/confidence/summary/evidence per pillar, deliberately no composite score) computed on demand from persisted observations bounded by an optional historical timestamp, so any assessment at time T is exactly reproducible with no new mutable table. `price_breadth`, `holder_behavior`, and — until at least 7 days of history accumulate — `liquidity` correctly report `unavailable` rather than a fabricated state. Live-verified end to end against real providers in the browser, including restart persistence and honest degradation of window-dependent derived metrics and regime pillars on a fresh database. See `docs/market-intelligence.md`. `market_breadth_advancing_pct` (would unblock the `price_breadth` regime pillar), wiring research modules to consume historical regime context, and the holder-behavior pillar (blocked on Coin Metrics catalog confirmation) remain planned. |
