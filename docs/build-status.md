...
| Market Intelligence Layer | Partial | Phase 0 through Phase 4 are complete: 20 metrics (OKX price/funding/open-interest, DefiLlama stablecoin market cap, CoinGecko globals, Fear & Greed, MVRV ratio + Z-Score, plus five Phase 3 derived-evidence metrics — annualized OKX funding, BTC MVRV percentile within Maybech's own history, stablecoin mcap 7d/30d change, and a BTC price/open-interest quadrant regime) across sentiment/valuation/price_breadth/derivatives/liquidity pillars, persisted with freshness/provider-health tracking, exposed through typed `/market/metrics`, `/market/series/{id}`, `/market/providers/status`, and rendered on a redesigned `frontend/app/analysis/overview/page.tsx` (pillar cards, per-metric freshness badges, provider-health strip, timeframe-controlled history charts with hover tooltips, honest empty states for `holder_behavior` and not-yet-accumulated history). Phase 4 added `GET /market/regime?at=`: a six-pillar regime map (state/confidence/summary/evidence per pillar, deliberately no composite score) computed on demand from persisted observations bounded by an optional historical timestamp, so any assessment at time T is exactly reproducible with no new mutable table. `price_breadth`, `holder_behavior`, and — until at least 7 days of history accumulate — `liquidity` correctly report `unavailable` rather than a fabricated state. Live-verified end to end against real providers in the browser, including restart persistence and honest degradation of window-dependent derived metrics and regime pillars on a fresh database. See `docs/market-intelligence.md`. Fear & Greed rolling-average/percentile derived metrics, wiring research modules to consume historical regime context, and the holder-behavior pillar (blocked on Coin Metrics catalog confirmation) remain planned. |
