...
1. Preserve in-progress Position rule edits across normal live polling. In the
...
2. Guard edits to enabled strategies in order-capable runtimes. With Demo armed
...
3. Do not mark confirmed Maybech entries as unexplained external exposure. The
...
4. Restore authenticated dashboard operation. A real Playwright Simulation run
...
5. Isolate malformed OKX instrument rows instead of aborting every Demo/Live
...
6. Expose continuous execution correctness health in every order-capable
...
7. Quarantine and deduplicate rejected fill evidence and notifications. After
...
8. Present account-mode-correct available collateral and unrealized PnL. The
...
9. Make a fresh Simulation workspace operable without violating exchange
...
10. Provide one consistent searchable SWAP selector across Strategy creation,
...
11. Prevent Next.js 16.2.9 Turbopack from terminating the dashboard development
...
12. Remove the persisted-theme hydration mismatch. The Playwright interaction
...
13. Localize all operator-facing protection lifecycle controls consistently in
...
14. Make trade history distinguish separate logical executions. The Simulation
...
15. Make the documented local frontend URL work cleanly in development. A real
