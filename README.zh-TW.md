...
在高度連動的加密貨幣市場中，最關鍵的彈性在於：任何條件的 `symbol` 都可以是
`"self"`（策略自身交易的商品），也可以是任何其他 OKX 商品代碼——跨商品條件
並不是特殊情況，只是同一棵樹裡的另一個節點。所以一個交易 ETH 的策略，可以
把進場條件建立在 BTC（或任何其他商品）的走勢上，再與自身的條件用
`and`/`or`/`not` 組合起來：
...
除了操作者自行組合的條件之外，每個策略的進場——不論交易哪個商品——在被實際
執行前，都還會再對照一個即時、永遠啟用的 BTC 市場狀態分類
（`direction` × `strength` × `impulse`）進行檢查，詳見
[`BTCRegimeActionPolicy`](src/trading/action_policy.py)。這道機制永遠只會
「阻擋」，不會「強制」進場。

策略訊號每一次由否轉是的邊界，都會在送出任何委託之前，先被持久化記錄成一筆
