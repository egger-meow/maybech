...
import { getMarketCandles, getSupportResistanceAnalysis, listLogicalPositions, promotePositionRiskStop, quoteInstrumentRisk, type InstrumentRiskQuoteRequest, type MarketCandlesResponse, type SupportResistanceAnalysisResponse } from "@/lib/api";
...
function RiskSizing({ instrument, bar, latestPrice, selectedStop }: { instrument: string; bar: string; latestPrice?: number | null; selectedStop?: number | null }) {
  ...
  const effectiveEntry = selectedPosition ? String(selectedPosition.entry_price) : entry || (latestPrice ? String(latestPrice) : "");
  const effectiveSide = selectedPosition?.side === "short" ? "short" : selectedPosition?.side === "long" ? "long" : side;
  ...
  const promote = async () => {
    if (!selectedPosition || !quote.data) return;
    const stopCondition = selectedPosition.close_conditions?.find((item) => item.purpose === "stop_loss" && item.enabled);
    if (!confirm(`Promote reviewed stop ${quote.data.stop_price} to logical position ${selectedPosition.id}?`)) return;
